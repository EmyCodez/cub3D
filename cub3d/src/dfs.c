...
/*   Updated: 2024/10/04 10:15:47 by adshafee         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int dfs_replace_spaces(t_map *map, int x, int y, int **visited) {
    ...
    if (x < 0 || y < 0 || x >= map->map_height || y >= (int)strlen(map->map_data[x])) {
    ...
    if (map->map_data[x][y] == ' ') {
        printf("Replacing space at (%d, %d) with '0'\n", x + 1, y + 1);
        map->map_data[x][y] = '1';
    }

    // Explore neighbors: Up, Down, Left, Right
    if (!dfs_replace_spaces(map, x + 1, y, visited)) return 0; // Down
    if (!dfs_replace_spaces(map, x - 1, y, visited)) return 0; // Up
    if (!dfs_replace_spaces(map, x, y + 1, visited)) return 0; // Right
    if (!dfs_replace_spaces(map, x, y - 1, visited)) return 0; // Left
...
int allocate_visited(int ***visited, int height, int width) {
    ...
    *visited = malloc(sizeof(int *) * height);
    ...
    while (i < height) {
        ...
        memset((*visited)[i], 0, sizeof(int) * width);  // Initialize to zero (unvisited)
...
int validate_and_replace_spaces(t_map *map) {
    ...
    while (i < map->map_height) {
        ...
        if (start_x != -1) break;  // Exit if starting position is found
    ...
    if (!dfs_replace_spaces(map, start_x, start_y, visited)) {
        for (int i = 0; i < map->map_height; i++) free(visited[i]);
        free(visited);
